...
SensorInsightsPatchDeviceRequestErrorBody: TypeAlias = (
    ManagementError400 | ManagementError | ManagementError403 | ManagementError404 | ManagementError500 | RawError
)


@dataclass(frozen=True, slots=True)
class _SensorInsightsPatchDeviceRequestError:
    def map(self, response: HttpResponse) -> SensorInsightsPatchDeviceRequestErrorBody:
...
sensor_insights_patch_device_request_error_mapper: Final[
    ErrorMapper[SensorInsightsPatchDeviceRequestErrorBody]
] = _SensorInsightsPatchDeviceRequestError()
