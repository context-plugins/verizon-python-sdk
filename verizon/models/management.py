...
from .enums.awareness_distance import AwarenessDistanceOrStr
from .event_position import EventPosition, EventPositionDict


class Management(SdkBaseModel):
    ...
    detection_time: int = Field(alias="detectionTime")
    """Timestamp in milliseconds since start of 2004 when event was first generated"""

    reference_time: int = Field(alias="referenceTime")
    """Timestamp in milliseconds since start of 2004 when the DENM message was generated."""

    event_position: EventPosition = Field(alias="eventPosition")
    awareness_distance: Optional[AwarenessDistanceOrStr] = Field(default=UNSET, alias="awarenessDistance")
    """Specifies how far the event is relevant to."""

    station_type: int = Field(alias="stationType")
    ...
    - 5 ``passengerCar`` - ITS-S mounted on small passenger vehicles as defined in UNECE/TRANS/WP.29/78/Rev.4 [16] class
        M1,
    ...
    - 8 ``heavyTruck`` - ITS-S mounted on Heavy Goods Vehicles as defined in UNECE/TRANS/WP.29/78/Rev.4 [16] class N2
        and N3,
    - 9 ``trailer`` - ITS-S mounted on an unpowered vehicle that is intended to be towed by a powered vehicle as defined
        in
                                UNECE/TRANS/WP.29/78/Rev.4 [16] class O,
    - 10 ``specialVehicles`` - ITS-S mounted on vehicles which have special purposes other than the above (e.g. moving
        road works vehicle),
    - 11 ``tram`` - ITS-S mounted on a vehicle which runs on tracks along public streets,
    - 12 ``lightVruVehicle`` - ITS-S carried by a human being traveling on light vehicle , incl. possible use of roller
        skates or skateboards (VRU profile 2),
    - 13 ``animal`` - ITS-S carried by an animal presenting a safety risk to other road users e.g. domesticated dog in a
        city or horse (VRU Profile 4),
    - 14 - reserved for future usage,
    - 15 ``roadSideUnit`` - ITS-S mounted on an infrastructure typically positioned outside of the drivable roadway
        (e.g. on a gantry, on a pole,
                                on a stationary road works trailer); the infrastructure is static during the entire
                                operation period of the ITS-S (e.g. no stop and go activity),
