...
from .enums.altitude_confidence import AltitudeConfidenceOrStr


class Altitude(SdkBaseModel):
    altitude_value: Optional[int] = Field(default=UNSET, alias="altitudeValue")
    ...
    - ``n`` (``n > -100000`` and ``n < 800000``) if the altitude is equal to or less than n x 0,01 metre and greater
        than (n-1) x 0,01 metre,
    - ``800000`` if the altitude greater than 7999,99 m,
    - ``800001`` if the information is not available."""

    altitude_confidence: Optional[AltitudeConfidenceOrStr] = Field(default=UNSET, alias="altitudeConfidence")
...
class AltitudeDict(TypedDict):
    altitude_value: NotRequired[int]
    altitude_confidence: NotRequired[AltitudeConfidenceOrStr]
